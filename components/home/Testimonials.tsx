...
export function Testimonials() {
  return (
    <section className="max-w-6xl mx-auto px-6 py-24">
      <Reveal>
        <p className="text-xs font-semibold text-[#6C4FD1] tracking-wide uppercase text-center">Loved by creators</p>
      ...
      <div className="mt-14 grid md:grid-cols-3 gap-6">
        {TESTIMONIALS.map((t, i) => (
          <Reveal key={t.name} delayMs={i * 90}>
            <div className="h-full rounded-2xl border border-black/10 p-7 bg-white">
              <div className="flex gap-0.5 text-[#F5B942]">
                {Array.from({ length: 5 }).map((_, s) => (
                  <Star key={s} size={14} fill="currentColor" strokeWidth={0} />
                ))}
              </div>
              <p className="mt-4 text-sm text-[#4A4750] leading-relaxed">&ldquo;{t.quote}&rdquo;</p>
              <div className="mt-6 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#17161B]">{t.name}</p>
                  <p className="text-xs text-[#4A4750]">{t.role}</p>
                </div>
              </div>
            </div>
          </Reveal>
